...
The general solution of the equation for forced SHM shown in @eq-ch5-forcedshm1 is a combination of these two regimes.

## The Transient Solution {#sec-ch5-transientsolution}

The solution to the transient component of the forced oscillator is identical to the solution of damped SHM (*i.e.* @eq-ch5-forcedshm1 with the right-hand side set to zero; identical to @eq-ch2-velocitydamp3). Its solution is shown in @eq-ch5-forcedshm2:
...
The amplitude (and hence the energy) of the system in the steady state depends on both the **amplitude** and the **frequency** of the driving force.  For a steady state, the solution to @eq-ch5-forcedshm1 is in the form shown in @eq-ch5-forcedshm3:

$$
x = A \mathrm{e}^{\mathrm{i}\left(\omega t - \delta \right)}
$${#eq-ch5-forcedshm3}

The terms $A$ and $\delta$ are defined in @eq-ch5-forcedshm4 and @eq-ch5-forcedshm5 below:
...
#| label: fig-ch5-resonancepowerfrequency1
#| fig-cap: "The power transferred to a system varies with the driving frequency. The power transferred to a system is proportional to $\\omega A^2$. The full-width at half-maximum value is designated $\\Delta \\omega$. Note that as the damping coefficient $b$ increases, there is a shift in maximum power transmission away from the natural frequency $\\omega_0$."
knitr::include_graphics("visualisations/ch5-forcedosc2.svg",auto_pdf = getOption("knitr.graphics.auto_pdf", TRUE))
```

If this system is damped, we also see a change in the "full-width at half-maximum" value.^[The "full-width at half maximum" (FWHM) is a term widely used in signals processing to describe the "spread" of a signal. A large FWHM indicates a broad signal over a range of frequencies, while a small FWHM indicates a sharp signal over a narrow band of frequencies.] This FWHM is designated by $\Delta \omega$ in @fig-ch5-resonancepowerfrequency1 and is related to the $Q$-factor of the system via @eq-ch5-resonancequality1:
...
We can also examine the phase of the oscillator's displacement $x$ relative to that of the driving force, $\delta$, shown in @fig-ch5-resonancephaseshift1.
...
#| label: fig-ch5-resonancephaseshift1
#| fig-cap: "The phase shift $\\delta$ of the observed oscillation varies relative to the driving frequency $\\omega$."
knitr::include_graphics("visualisations/ch5-forcedosc3.svg",auto_pdf = getOption("knitr.graphics.auto_pdf", TRUE))
```
...
As with the damped oscillator explored in @sec-ch2-dampedoscillations, the full solution of the forced oscillator is found through the combination of the "transient state" (a free, damped system) and the "steady state" (fully forced oscillations).  This is illustrated in @eq-ch5-forcedoscillfullsoln1:
...
The terms in @eq-ch5-forcedoscillfullsoln1 are as follows:

* $\delta$ is given by @eq-ch5-forcedshm5;
* $A$ given by @eq-ch5-forcedshm4;
...
We can simplify our view of @eq-ch5-forcedoscillfullsoln1 by applying initial conditions whereby the displacement $x = 0$ at time $t = 0$. Therefore:

$$
0 = A_0 \mathrm{e}^{\mathrm{i} \left( \delta^\prime \right)} + A \mathrm{e}^{-\mathrm{i}\delta}
$$

...or, to rearrange, we obtain the form shown in @eq-ch5-forcedoscillfullsoln2:

$$
A_0 = -A \mathrm{e}^{-\mathrm{i}\left( \delta + \delta^\prime \right)}
$${#eq-ch5-forcedoscillfullsoln2}

As with any complex representation, we now compare the **real** and **imaginary** parts of the solution.  Remember that @eq-ch5-forcedoscillfullsoln2 expands *via* De Moivre's theorem to:

$$
A_0 = -A \left( \cos \left( \delta + \delta^\prime \right) - \mathrm{i} \sin \left( \delta + \delta^\prime \right)  \right)
$${#eq-ch5-forcedoscillfullsoln3}

The imaginary component of @eq-ch5-forcedoscillfullsoln3 reduces to zero (the solution, $A_0$ is fully real):
...
We now examine the 'real' component of @eq-ch5-forcedoscillfullsoln3, equal to $A_0$:
...
We can now revisit @eq-ch5-forcedoscillfullsoln1, now that we have values for $\delta$, $\delta^\prime$ and $A_0$ under our starting conditions ( $x = 0$ when $t = 0$):
...
Finally, we can find the actual displacement of the oscillator by examining the real component of @eq-ch5-forcedoscillationfullsoln6, summarised in @eq-ch5-forcedoscillationfullsoln7):
...
We can visualise the amplitude of a forced oscillator, as shown in @fig-ch5-forcedoscilfull1.  At very low frequencies, the oscillation has the same amplitude as the driving oscillation. This rapidly increases under resonance to a peak, but then drops rapidly to a value towards zero.
...
#| label: fig-ch5-forcedoscilfull1
#| fig-cap: "The amplitude of a forced oscillation varies with driving frequency; close to the driving amplitude at very low frequencies, close to zero at very high frequencies, with a peak occurring under resonant conditions. Note that the position of this peak varies with the damping coefficient $b$."
knitr::include_graphics("visualisations/ch5-forcedosc1.svg",auto_pdf = getOption("knitr.graphics.auto_pdf", TRUE))
```
...
#| label: fig-ch5-forcedosc4
#| fig-cap: "When the frequency of the driving oscillation $\\omega$ is very different to the natural frequency of the system we see two regimes; firstly, where $\\omega \\ll \\omega_0$ (blue), the natural frequency decays and we are left to observe the driving frequency at a large amplitude; when $\\omega \\gg \\omega_0$ (orange), we see a much smaller amplitude oscillation at close to the driving frequency."
knitr::include_graphics("visualisations/ch5-forcedosc4.svg",auto_pdf = getOption("knitr.graphics.auto_pdf", TRUE))
```

In the special case when $\omega = \omega_0$ and the damping coefficient $b$ is small (allowing the damped frequency $\omega^\prime$ to be approximately equal to the driving frequency $\omega$), we can rewrite the expression of the displacement (@eq-ch5-forcedoscillationfullsoln7) as shown in @eq-ch5-forcedoscillationfullsoln8:

$$
\mathrm{Re}(x) = A  \cos (\omega_0 t - \delta) \left[ 1 - \mathrm{e}^{- \left(\frac{b}{2m}\right)t}  \right]
$${#eq-ch5-forcedoscillationfullsoln8}

We can see that the oscillation will converge to a maximum value, at which point the energy put into the system becomes equal to the energy dissipated by the system through damping, as shown in @fig-ch5-resonancedisplacement1:
...
#| label: fig-ch5-resonancedisplacement1
#| fig-cap: "When the frequency of the driving oscillation $\\omega = \\omega_0$ (blue), we see the amplitude grow to a plateau at which point the energy put into the system is equal to the energy losses from the system."
knitr::include_graphics("visualisations/ch5-forcedosc5.svg",auto_pdf = getOption("knitr.graphics.auto_pdf", TRUE))
```
...
For the sake of convenience, we will consider the "real" component of the equation for forced SHM shown in @eq-ch5-forcedshm1 (the left-hand-side is entirely "real", so the "imaginary" component reduces to zero anyway):

$$
m \frac{\mathrm{d}^2 x}{\mathrm{d}t^2} + b \frac{\mathrm{d}x}{\mathrm{d}t}+ kx = F_0 \cos \omega t
$${#eq-ch5-oscillatorenergy1}

We also know that the steady state solution for the displacement $x$ in SHM is as given in @eq-shm-general-positionx1. We can then find an expression for the velocity $v$ by finding the first derivative of this expression (@eq-ch5-oscillatorenergy2):
...
Evaluation of this integral gives us the end result shown in @eq-ch5-oscillatorenergy4:

$$
E_{\mathrm{in}} = \frac{1}{2}\omega A F_0 T \sin \delta
$${#eq-ch5-oscillatorenergy4}

If we recall that $\delta = \frac{\pi}{2}$ at resonance, substitution of this value into @eq-ch5-oscillatorenergy4 tells us that the maximum energy is transferred to the oscillator when driven at a resonant frequency - in line with our expectations.

If we now use the definition of $\delta$ in @eq-ch5-forcedshm5, we can use trigonometry and the expression for $A$ in @eq-ch5-forcedshm4 to give the expression  $\sin \delta = \frac{Ab\omega}{F_0}$; we can also use the fact that the period of oscillation $T$ and the angular frequency $\omega$ are related by @eq-ch1-definingomega1 ($T = \frac{2\pi}{\omega}$), and simplify the expression for the energy input for the forced oscillator as shown in @eq-ch5-oscillatorenergy5:
...
To now determine the amount of energy, we follow a similar process as followed in @sec-ch5-energyinputdrivenosc. This time however we need to determine the work done by the oscillator on the damping force during one cycle.  This integral is set up as follows:
...
As before, we have the result that $v = -\omega A \sin(\omega t - \delta)$ (@eq-ch5-oscillatorenergy2), and following through the integration in a similar manner as in the previous section we are led to the result in @eq-ch5-oscillatorenergy6 for a **driven damped oscillator** at **steady state**.
